...
(define compose$
  (lambda (f$ g$)
   (lambda (x cont1)
    (f$ x (lambda (cont2) (g$ cont2 cont1)))
    )
 )
  )
...
(define generate-pi-approximations
  (lambda ()
   ; (reduce3-lzl (lambda (acc x) (+ acc (* 8(/ 1 (* x (+ x 2)))))) 0   (integers-steps-from 1 4))
    (reduce3-lzl + 0 (lzl-map (lambda (x) (* 8(/ 1 (* x (+ x 2))))) (integers-steps-from 1 4) ))
   ))
