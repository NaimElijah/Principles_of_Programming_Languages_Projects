...
import {all, append, chain, concat, equals, map, sort, uniq, zip, filter, includes, slice} from "ramda";
...
export type PredTExp = {tag: "PredTExp"; predType: TExp;};
export const makePredTExp = (predType: TExp):PredTExp => ({tag: "PredTExp", predType:predType})
export const isPredTExp = (x:any): x is PredTExp => x.tag === "PredTExp";

// proc-te(param-tes: list(te), return-te: te)
export type ProcTExp = { tag: "ProcTExp"; paramTEs: TExp[]; returnTE: TExp | PredTExp; };
//export type PredProcTExp = { tag: "ProcTExp"; paramTEs: TExp[]; returnTE: PredTExp; };
export const makeProcTExp = (paramTEs: TExp[], returnTE: TExp |  PredTExp): ProcTExp =>
...
//export const isPredProcTExp = (x:any): x is PredProcTExp => (x.tag === "ProcTExp" && isPredTExp(x.returnTE));
export const procTExpComponents = (pt: ProcTExp): TExp[] =>
    [...pt.paramTEs, isPredTExp(pt.returnTE)?pt.returnTE.predType:pt.returnTE];
...
export const makeInterTExp = (tes: TExp[]): TExp => dnf(normalizeInter({tag: "InterTExp", components: flattenInter(tes)}));
export const isInterTExp = (x: any): x is InterTExp => x.tag === "InterTExp";

//export type DiffTExp =  { tag: "DiffTExp"; components: TExp};

const diffHelperUnionUnion = (te1: UnionTExp,te2: UnionTExp) : TExp =>
    makeUnionTExp(filter((x) => !includes(x,te2.components),te1.components));

const diffHelperUnionNotUnion = (te1: UnionTExp,te2: TExp) : TExp =>
    makeUnionTExp(filter((x) => !equals(x,te2),te1.components));

export const makeDiffTExp = (te1: TExp, te2:TExp) : TExp =>
    isUnionTExp(te1)? isUnionTExp(te2)? diffHelperUnionUnion(te1,te2): diffHelperUnionNotUnion(te1,te2):
        isUnionTExp(te2)? includes(te1,te2.components)? makeNeverTExp() : te1
...
const normalizeInter = (ite: InterTExp): TExp =>
    isEmpty(ite.components)? makeNeverTExp():
        includes(makeNeverTExp(),ite.components)? makeNeverTExp():
            (ite.components.length===1)? ite.components[0] : removeAny(ite);

const removeAny = (ite : InterTExp) :TExp =>
{
    const temp = removeAnyHelper(ite.components)
    return temp.length==ite.components.length? ite:temp.length==1? temp[0] : makeInterTExp(temp);
}
const removeAnyHelper = (tes: TExp[]) :TExp[] =>
    (tes.length>0)? isAnyTExp(tes[0])? [...removeAnyHelper(tes.slice(1))]:
        [tes[0],...removeAnyHelper(tes.slice(1))]: [];
...
const flattenInter = (tes: TExp[]): TExp[] =>
    (tes.length>0)?
        isInterTExp(tes[0]) ? [...tes[0].components, ...flattenSortInter(tes.slice(1))] :
...
export const isSubType = (te1: TExp, te2: TExp): boolean =>
    ...
    isAnyTExp(te1)? false : isAtomicTExp(te1) ? equals(te1, te2) :
    false;
...
export const checkProcTExps = (te1: ProcTExp, te2: ProcTExp): boolean => 
    ...
    isSubType(isPredTExp(te1.returnTE)? makeBoolTExp():te1.returnTE, isPredTExp(te2.returnTE)? makeBoolTExp():te2.returnTE) &&
...
const parseInterTExp = (texps: Sexp[]) : Result<TExp> =>
    mapv(mapResult(parseTExp, texps.slice(1)),(tes: TExp[]) => makeInterTExp(tes));
...
const parseProcTExp = (texps: Sexp[]): Result<ProcTExp> => {
    ...
    return (pos === -1)  ? makeFailure(`Procedure type expression without -> - ${format(texps)}`) :
           ...
           bind(parseTupleTExp(texps.slice(0, pos)), (args: TExp[]) =>
               texps[pos + 1] === "is?"? mapv(mapv(parseTExp(texps[pos + 2]),(t) => makePredTExp(t)), (returnTE: PredTExp) =>
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const parenthesizeInter = (tes: string[]): string =>
        (tes.length == 1) ? tes[0] :  // (union T) -> T
            `(inter ${tes[0]} ${parenthesizeUnion(tes.slice(1))})`
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isProcTExp(x) ? bind(unparseTuple(x.paramTEs), (paramTEs: string[]) =>
                            mapv(isPredTExp(x.returnTE)? unparsePredTExp(x.returnTE) : unparseTExp(x.returnTE), (returnTE: string) =>
                                [...paramTEs, '->', returnTE])) :
    ...
    const unparsePredTExp = (predTE: PredTExp): Result<string> =>
        bind(unparseTExp(predTE.predType) , (x) => makeOk(`is? ${x}`));
