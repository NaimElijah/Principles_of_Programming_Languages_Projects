...
import {
    ...
    Parsed, PrimOp, ProcExp, Program, StrExp, parseL5Program, VarDecl, makeVarDecl, isVarDecl, makeVarRef
} from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import {
    isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
    parseTE, unparseTExp, makeUnionTExp,
    BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isSubType, isPredTExp, ProcTExp, PredTExp, isAnyTExp, makeDiffTExp
} from "./TExp";
...
export const checkCompatibleType = (te1: TExp, te2: TExp, exp: Exp): Result<true> =>
  isSubType(te1, te2) ? makeOk(true) : isAnyTExp(te1)||isAnyTExp(te2)? makeOk(true):
...
const isTypePredApp = (e: Exp, tenv: TEnv): Result<VarDecl> =>
    isAppExp(e)? bind(typeofExp(e.rator,tenv), (rator) => isTypePredAppHelper(rator,e.rands[0])) : makeFailure("");

const isTypePredAppHelper = (exp: TExp, operand: Exp): Result<VarDecl> =>
    (isProcTExp(exp)&& isPredTExp(exp.returnTE) && isVarRef(operand))?
        makeOk(makeVarDecl(operand.var,exp.returnTE.predType)) : makeFailure("");
// const isTypePredAppHelper2 = (appExp: AppExp, operand: Exp, tenv: TEnv): Result<VarDecl> =>
//     (isProcExp(appExp.rator)&& isPredTExp(appExp.rator.returnTE) && isVarRef(operand))?
//         makeOk(makeVarDecl(operand.var,appExp.rator.returnTE.predType)) : makeFailure("");



export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> =>
    either(
        bind (isTypePredApp(ifExp.test, tenv), (predType : VarDecl) => typeOfPredIf(ifExp, tenv,predType)),
        makeOk,
        () => typeofIfNormal(ifExp, tenv));


const typeOfPredIf = (ifExp: IfExp, tenv: TEnv,predType:VarDecl): Result<TExp> =>
{
    const testTE = typeofExp(ifExp.test, tenv);
    const thenTE = typeofExp(ifExp.then, makeExtendTEnv([predType.var],[predType.texp],tenv));
    const altTE =  bind(typeofExp(makeVarRef(predType.var),tenv) ,(expType) => typeofExp(ifExp.alt, makeExtendTEnv([predType.var],[makeDiffTExp(expType,predType.texp)],tenv)));
    const constraint1 = bind(testTE, testTE => checkCompatibleType(testTE, makeBoolTExp(), ifExp));
    // L51
    const constraint2 = bind(thenTE, (thenTE: TExp) =>
        bind(altTE, (altTE: TExp) =>
...
export const typeofProc = (proc: ProcExp, tenv: TEnv): Result<TExp> =>
    isPredTExp(proc.returnTE)?
    typeOfPredProc(proc.returnTE,proc,tenv):typeOfNonPredProc(proc.returnTE,proc,tenv);


const typeOfPredProc = (returnTE: PredTExp,proc: ProcExp, tenv: TEnv): Result<TExp> =>
{
    const argsTEs = map((vd) => vd.texp, proc.args);
    const extTEnv = makeExtendTEnv(map((vd) => vd.var, proc.args), argsTEs, tenv);
    const temp1 = typeofExps(proc.body, extTEnv);
    const constraint1 = bind(typeofExps(proc.body, extTEnv), (body: TExp) =>
        checkCompatibleType(body, makeBoolTExp(), proc));
    return bind(constraint1, _ => makeOk(makeProcTExp(argsTEs, returnTE)));
}

const typeOfNonPredProc = (returnTE: TExp,proc: ProcExp, tenv: TEnv): Result<TExp> =>
{
    const argsTEs = map((vd) => vd.texp, proc.args);
    const extTEnv = makeExtendTEnv(map((vd) => vd.var, proc.args), argsTEs, tenv);
    const constraint1 = bind(typeofExps(proc.body, extTEnv), (body: TExp) =>
        checkCompatibleType(body, returnTE, proc));
    return bind(constraint1, _ => makeOk(makeProcTExp(argsTEs, returnTE)));
}
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> =>
    bind(typeofExp(app.rator, tenv), (ratorTE: TExp) => {
        ...
        return isPredTExp(ratorTE.returnTE) ?  typeOfPredProcInApp(ratorTE.returnTE,ratorTE,app,tenv) :  typeOfNonPredProcInApp(ratorTE.returnTE,ratorTE,app,tenv);
...
const typeOfNonPredProcInApp = (returnTE:TExp ,ratorTE: ProcTExp,app:AppExp,tenv:TEnv): Result<TExp> =>
{

    const constraints = zipWithResult((rand, trand: TExp) =>
            bind(typeofExp(rand, tenv), (typeOfRand: TExp) =>
            checkCompatibleType(typeOfRand, trand, app)),
        app.rands, ratorTE.paramTEs);
     return bind(constraints, _ => makeOk(returnTE));
}
const typeOfPredProcInApp = (returnTE:PredTExp ,ratorTE: ProcTExp,app:AppExp,tenv:TEnv): Result<TExp> =>
{
    const constraints = zipWithResult((rand, trand: TExp) =>
            bind(typeofExp(rand, tenv), (typeOfRand: TExp) =>
                checkCompatibleType(typeOfRand, trand, app)),
        app.rands, ratorTE.paramTEs);
    //const constraint2 = checkCompatibleType(returnTE.predType,makeBoolTExp(),app);
...
// then type<(letrec((p1 (lambda (x11 ... x1n1) body1)) ...) body)>(tenv-body) = t
 export const typeofLetrec = (exp: LetrecExp, tenv: TEnv): Result<TExp> => {
     ...
     const constraints = bind(types, (types: TExp[]) =>
                             zipWithResult((typeI, ti) => isPredTExp(ti)? checkCompatibleType(typeI, makeBoolTExp(), exp): checkCompatibleType(typeI, ti, exp), types, tis));
