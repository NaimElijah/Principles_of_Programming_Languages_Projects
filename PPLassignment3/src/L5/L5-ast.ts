...
export type ProcExp = {tag: "ProcExp"; args: VarDecl[], body: CExp[]; returnTE: TExp |PredTExp }
export const makeProcExp = (args: VarDecl[], body: CExp[], returnTE: TExp |PredTExp): ProcExp =>
...
const parseProcExp = (vars: Sexp, rest: Sexp[]): Result<ProcExp> => {
    if (isArray(vars)) {
        const args = vars.length>1&&vars[1]===":"? mapResult(parseVarDecl, [vars]) : mapResult(parseVarDecl, vars);
        const body = mapResult(parseL5CExp, rest[0] === ":" ? rest[1] === "is?"? rest.slice(3) : rest.slice(2) : rest);
        const returnTE: Result<TExp|PredTExp> = rest[0] === ":" ?
            rest[1] === "is?"? bind(parseTExp(rest[2]), (x) => makeOk(makePredTExp(x))) : parseTExp(rest[1]) : makeOk(makeFreshTVar());
        return bind(args, (args: VarDecl[]) =>
                    bind(body, (body: CExp[]) =>
                        mapv(returnTE, (returnTE: TExp| PredTExp) =>
...
const unparsePredReturn = (predTE: PredTExp): Result<string> =>
    mapv(unparseTExp(predTE.predType), (te:string) => ': is? ${te}')
...
const unparseProcExp = (pe: ProcExp): Result<string> =>
    isPredTExp(pe.returnTE)? unparsePredProcExp(pe, pe.returnTE) : unparseNonPredProcExp(pe, pe.returnTE)


const unparseNonPredProcExp = (pe : ProcExp, returnTE : TExp) : Result<string>=>
    bind(mapResult(unparseVarDecl, pe.args), (vds: string[]) =>
        bind(unparseReturn(returnTE), (ret: string) =>
            mapv(unparseLExps(pe.body), (body: string) =>
                `(lambda (${join(" ", vds)})${ret} ${body})`)));

const unparsePredProcExp = (pe : ProcExp, returnTE : PredTExp): Result<string> =>
    bind(mapResult(unparseVarDecl, pe.args), (vds: string[]) =>
        bind(unparsePredReturn(returnTE), (ret: string) =>
            mapv(unparseLExps(pe.body), (body: string) =>
                `(lambda (${join(" ", vds)})${ret} ${body})`)));
